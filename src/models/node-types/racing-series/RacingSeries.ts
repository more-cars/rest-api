...
export const RacingSeries = {
    async create(data: RacingSeriesInput): Promise<RacingSeriesNode> {
        const input = convertInputData(data)
        const result = await createDbNode(DbNodeType.RacingSeries, input)
    ...
    async findAll(options: NodeCollectionConstraints = {}): Promise<RacingSeriesNode[]> {
        ...
        const nodesDb = await fetchNodesFromDb(DbNodeType.RacingSeries, getDbQueryCollectionParams(options))
    ...
    async update(id: number, data: RacingSeriesInput): Promise<RacingSeriesNode> {
        const node = await getNodeById(id)
        ...
        const input = convertInputData(data)
        const result = await updateDbNode(DbNodeType.RacingSeries, id, input)
