...
export const MagazineIssue = {
    async create(data: MagazineIssueInput): Promise<MagazineIssueNode> {
    ...
    async findAll(options: NodeCollectionConstraints = {}): Promise<MagazineIssueNode[]> {
        ...
        const nodesDb = await fetchNodesFromDb(DbNodeType.MagazineIssue, getDbQueryCollectionParams(options))
    ...
    async deleteReviewedCarModelVariantWithRatingRelationship(magazineIssueId: number, ratingId: number) {
        // checking that both nodes exist -> exception is thrown if not
        await MagazineIssue.findById(magazineIssueId)
        await Rating.findById(ratingId)

        const relationship = await getSpecificRel(magazineIssueId, ratingId, RelType.MagazineIssueReviewedCarModelVariantWithRating)
        if (!relationship) {
            throw new RelNotFoundError(RelType.MagazineIssueReviewedCarModelVariantWithRating, magazineIssueId, ratingId)
        }

        await deleteSpecificRel(magazineIssueId, ratingId, RelType.MagazineIssueReviewedCarModelVariantWithRating)
    },
    ...
    async createCoversRacingEventRelationship(magazineIssueId: number, racingEventId: number) {
        ...
        const existingRelation = await getSpecificRel(magazineIssueId, racingEventId, RelType.MagazineIssueCoversRacingEvent)
        if (existingRelation) {
            throw new RelAlreadyExistsError(RelType.MagazineIssueCoversRacingEvent, magazineIssueId, racingEventId)
        }


        const createdRelationship = await createRel(magazineIssueId, racingEventId, RelType.MagazineIssueCoversRacingEvent)
        if (!createdRelationship) {
            throw new Error('Relationship could not be created')
        }
    ...
    async getAllCoversRacingEventRelationships(magazineIssueId: number) {
        // checking that the node exists -> exception is thrown if not
        await MagazineIssue.findById(magazineIssueId)
    ...
    async createDocumentsLapTimeRelationship(magazineIssueId: number, lapTimeId: number) {
        // checking that both nodes exist -> exception is thrown if not
        await MagazineIssue.findById(magazineIssueId)
        await LapTime.findById(lapTimeId)

        const existingRelation = await getSpecificRel(magazineIssueId, lapTimeId, RelType.MagazineIssueDocumentsLapTime)
        if (existingRelation) {
            throw new RelAlreadyExistsError(RelType.MagazineIssueDocumentsLapTime, magazineIssueId, lapTimeId)
        }

        await deleteIncomingRel(lapTimeId, RelType.MagazineIssueDocumentsLapTime)

        const createdRelationship = await createRel(magazineIssueId, lapTimeId, RelType.MagazineIssueDocumentsLapTime)
        if (!createdRelationship) {
            throw new Error('Relationship could not be created')
        }
    ...
    async getAllDocumentsLapTimeRelationships(magazineIssueId: number) {
        // checking that the node exists -> exception is thrown if not
        await MagazineIssue.findById(magazineIssueId)

        return getAllRels(magazineIssueId, RelType.MagazineIssueDocumentsLapTime)
    },

    async deleteDocumentsLapTimeRelationship(magazineIssueId: number, lapTimeId: number) {
        // checking that both nodes exist -> exception is thrown if not
        await MagazineIssue.findById(magazineIssueId)
        await LapTime.findById(lapTimeId)

        const relationship = await getSpecificRel(magazineIssueId, lapTimeId, RelType.MagazineIssueDocumentsLapTime)
        if (!relationship) {
            throw new RelNotFoundError(RelType.MagazineIssueDocumentsLapTime, magazineIssueId, lapTimeId)
        }

        await deleteSpecificRel(magazineIssueId, lapTimeId, RelType.MagazineIssueDocumentsLapTime)
    },
