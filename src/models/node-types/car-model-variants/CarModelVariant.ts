...
export const CarModelVariant = {
    async create(data: CreateCarModelVariantInput): Promise<CarModelVariantNode> {
        ...
        const result = await createNeo4jNode(DbNodeType.CarModelVariant, input)
    ...
    async findAll(options: NodeCollectionConstraints = {}): Promise<CarModelVariantNode[]> {
        ...
        const nodesDb = await fetchNodesFromDb(DbNodeType.CarModelVariant, getDbQueryCollectionParams(options))
    ...
    async createAchievedSessionResultRelationship(carModelVariantId: number, sessionResultId: number) {
        ...
        await deleteIncomingRel(sessionResultId, RelType.CarModelVariantAchievedSessionResult)
    ...
    async createAchievedLapTimeRelationship(carModelVariantId: number, lapTimeId: number) {
        ...
        await deleteIncomingRel(lapTimeId, RelType.CarModelVariantAchievedLapTime)
    ...
    async createReviewedByMagazineIssueWithRatingRelationship(carModelVariantId: number, ratingId: number) {
        ...
        await deleteIncomingRel(ratingId, RelType.CarModelVariantReviewedByMagazineIssueWithRating)
    ...
    async createHasScaleModelRelationship(carModelVariantId: number, modelCarId: number) {
        ...
        await deleteIncomingRel(modelCarId, RelType.CarModelVariantHasScaleModel)
    ...
    async createHasPriceRelationship(carModelVariantId: number, priceId: number) {
        ...
        await deleteIncomingRel(priceId, RelType.CarModelVariantHasPrice)
    ...
    async deleteHasPriceRelationship(carModelVariantId: number, priceId: number) {
        // checking that both nodes exist -> exception is thrown if not
        await CarModelVariant.findById(carModelVariantId)
        await Price.findById(priceId)

        const relationship = await getSpecificRel(carModelVariantId, priceId, RelType.CarModelVariantHasPrice)
        if (!relationship) {
            throw new RelNotFoundError(RelType.CarModelVariantHasPrice, carModelVariantId, priceId)
        }
