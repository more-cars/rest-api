...
export const RacingEvent = {
    ...
    async findAll(options: NodeCollectionConstraints = {}): Promise<RacingEventNode[]> {
        ...
        const nodesDb = await fetchNodesFromDb(DbNodeType.RacingEvent, getDbQueryCollectionParams(options))
    ...
    async update(id: number, data: RacingEventInput): Promise<RacingEventNode> {
        const node = await getNodeById(id)
    ...
    async createCoveredByMagazineIssueRelationship(racingEventId: number, magazineIssueId: number) {
        // checking that both nodes exist -> exception is thrown if not
        await RacingEvent.findById(racingEventId)
        await MagazineIssue.findById(magazineIssueId)

        const existingRelation = await getSpecificRel(racingEventId, magazineIssueId, RelType.RacingEventCoveredByMagazineIssue)
        if (existingRelation) {
            throw new RelAlreadyExistsError(RelType.RacingEventCoveredByMagazineIssue, racingEventId, magazineIssueId)
        }


        const createdRelationship = await createRel(racingEventId, magazineIssueId, RelType.RacingEventCoveredByMagazineIssue)
        if (!createdRelationship) {
            throw new Error('Relationship could not be created')
        }
    ...
    async deleteCoveredByMagazineIssueRelationship(racingEventId: number, magazineIssueId: number) {
        // checking that both nodes exist -> exception is thrown if not
        await RacingEvent.findById(racingEventId)
        await MagazineIssue.findById(magazineIssueId)

        const relationship = await getSpecificRel(racingEventId, magazineIssueId, RelType.RacingEventCoveredByMagazineIssue)
        if (!relationship) {
            throw new RelNotFoundError(RelType.RacingEventCoveredByMagazineIssue, racingEventId, magazineIssueId)
        }

        await deleteSpecificRel(racingEventId, magazineIssueId, RelType.RacingEventCoveredByMagazineIssue)
    },
