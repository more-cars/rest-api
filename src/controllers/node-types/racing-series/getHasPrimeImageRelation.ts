...
export async function getHasPrimeImageRelation(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelation = await RacingSeries.getHasPrimeImageRelationship(racingSeriesId)
        ...
        if (e instanceof NodeNotFoundError) {
            ...
            const marshalledData = marshalEmptyRelation(ControllerNodeType.RacingSeries, racingSeriesId, RelationType.RacingSeriesHasPrimeImage)
            return sendResponse200(marshalledData, res)
        } else {
            console.error(e)
