...
export async function getHasMainVideoRelation(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelation = await RacingSession.getHasMainVideoRelationship(racingSessionId)
        ...
        if (e instanceof NodeNotFoundError) {
            ...
            const marshalledData = marshalEmptyRelation(ControllerNodeType.RacingSession, racingSessionId, RelationType.RacingSessionHasMainVideo)
            return sendResponse200(marshalledData, res)
        } else {
            console.error(e)
