...
export async function getAllHasImageRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await MagazineIssue.getAllHasImageRelationships(magazineIssueId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.MagazineIssue, magazineIssueId, RelationType.MagazineIssueHasImage)
