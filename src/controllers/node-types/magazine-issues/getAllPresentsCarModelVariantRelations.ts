...
export async function getAllPresentsCarModelVariantRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await MagazineIssue.getAllPresentsCarModelVariantRelationships(magazineIssueId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.MagazineIssue, magazineIssueId, RelationType.MagazineIssuePresentsCarModelVariant)
