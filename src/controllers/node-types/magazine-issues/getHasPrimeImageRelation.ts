...
export async function getHasPrimeImageRelation(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelation = await MagazineIssue.getHasPrimeImageRelationship(magazineIssueId)
        ...
        if (e instanceof NodeNotFoundError) {
            ...
            const marshalledData = marshalEmptyRelation(ControllerNodeType.MagazineIssue, magazineIssueId, RelationType.MagazineIssueHasPrimeImage)
            return sendResponse200(marshalledData, res)
        } else {
            console.error(e)
