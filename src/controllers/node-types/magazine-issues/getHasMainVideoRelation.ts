...
export async function getHasMainVideoRelation(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelation = await MagazineIssue.getHasMainVideoRelationship(magazineIssueId)
        ...
        if (e instanceof NodeNotFoundError) {
            ...
            const marshalledData = marshalEmptyRelation(ControllerNodeType.MagazineIssue, magazineIssueId, RelationType.MagazineIssueHasMainVideo)
            return sendResponse200(marshalledData, res)
        } else {
            console.error(e)
