...
export async function createDocumentsLapTimeRelation(req: express.Request, res: express.Response) {
    const magazineIssueId = parseInt(req.params.magazineIssueId)
    const lapTimeId = parseInt(req.params.lapTimeId)

    try {
        const modelRelation = await MagazineIssue.createDocumentsLapTimeRelationship(magazineIssueId, lapTimeId)
