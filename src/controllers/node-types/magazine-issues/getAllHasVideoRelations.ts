...
export async function getAllHasVideoRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await MagazineIssue.getAllHasVideoRelationships(magazineIssueId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.MagazineIssue, magazineIssueId, RelationType.MagazineIssueHasVideo)
