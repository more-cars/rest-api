...
export async function getAllHasLapTimeRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await SessionResult.getAllHasLapTimeRelationships(sessionResultId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.SessionResult, sessionResultId, RelationType.SessionResultHasLapTime)
