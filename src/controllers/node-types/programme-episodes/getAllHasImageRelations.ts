...
export async function getAllHasImageRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await ProgrammeEpisode.getAllHasImageRelationships(programmeEpisodeId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.ProgrammeEpisode, programmeEpisodeId, RelationType.ProgrammeEpisodeHasImage)
