...
export async function getHasPrimeImageRelation(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelation = await ProgrammeEpisode.getHasPrimeImageRelationship(programmeEpisodeId)
        ...
        if (e instanceof NodeNotFoundError) {
            ...
            const marshalledData = marshalEmptyRelation(ControllerNodeType.ProgrammeEpisode, programmeEpisodeId, RelationType.ProgrammeEpisodeHasPrimeImage)
            return sendResponse200(marshalledData, res)
        } else {
