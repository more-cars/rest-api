...
export async function getAllFeaturesCarModelVariantRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await ProgrammeEpisode.getAllFeaturesCarModelVariantRelationships(programmeEpisodeId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.ProgrammeEpisode, programmeEpisodeId, RelationType.ProgrammeEpisodeFeaturesCarModelVariant)
