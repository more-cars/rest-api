...
export async function getAllBelongsToNodeRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await Video.getAllBelongsToNodeRelationships(videoId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.Video, videoId, RelationType.VideoBelongsToNode)
