...
export async function getIsFollowedByEventRelation(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelation = await RacingEvent.getIsFollowedByEventRelationship(racingEventId)
        ...
        if (e instanceof NodeNotFoundError) {
            ...
            const marshalledData = marshalEmptyRelation(ControllerNodeType.RacingEvent, racingEventId, RelationType.RacingEventIsFollowedByEvent)
            return sendResponse200(marshalledData, res)
        } else {
            console.error(e)
