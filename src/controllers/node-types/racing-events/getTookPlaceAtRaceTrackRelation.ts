...
export async function getTookPlaceAtRaceTrackRelation(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelation = await RacingEvent.getTookPlaceAtRaceTrackRelationship(racingEventId)
        ...
        if (e instanceof NodeNotFoundError) {
            ...
            const marshalledData = marshalEmptyRelation(ControllerNodeType.RacingEvent, racingEventId, RelationType.RacingEventTookPlaceAtRaceTrack)
            return sendResponse200(marshalledData, res)
        } else {
            console.error(e)
