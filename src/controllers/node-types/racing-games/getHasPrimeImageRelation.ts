...
export async function getHasPrimeImageRelation(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelation = await RacingGame.getHasPrimeImageRelationship(racingGameId)
        ...
        if (e instanceof NodeNotFoundError) {
            ...
            const marshalledData = marshalEmptyRelation(ControllerNodeType.RacingGame, racingGameId, RelationType.RacingGameHasPrimeImage)
            return sendResponse200(marshalledData, res)
        } else {
            console.error(e)
