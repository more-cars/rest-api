...
export async function getAllFeaturesTrackLayoutRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await RacingGame.getAllFeaturesTrackLayoutRelationships(racingGameId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.RacingGame, racingGameId, RelationType.RacingGameFeaturesTrackLayout)
