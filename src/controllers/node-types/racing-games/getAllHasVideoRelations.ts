...
export async function getAllHasVideoRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await RacingGame.getAllHasVideoRelationships(racingGameId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.RacingGame, racingGameId, RelationType.RacingGameHasVideo)
