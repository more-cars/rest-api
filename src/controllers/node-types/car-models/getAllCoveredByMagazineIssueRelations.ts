...
export async function getAllCoveredByMagazineIssueRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await CarModel.getAllCoveredByMagazineIssueRelationships(carModelId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.CarModel, carModelId, RelationType.CarModelCoveredByMagazineIssue)
