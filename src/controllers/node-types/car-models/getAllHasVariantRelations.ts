...
export async function getAllHasVariantRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await CarModel.getAllHasVariantRelationships(carModelId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.CarModel, carModelId, RelationType.CarModelHasVariant)
