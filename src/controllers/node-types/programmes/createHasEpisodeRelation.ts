...
export async function createHasEpisodeRelation(req: express.Request, res: express.Response) {
    const programmeId = parseInt(req.params.programmeId)
    const programmeEpisodeId = parseInt(req.params.programmeEpisodeId)

    try {
        const modelRelation = await Programme.createHasEpisodeRelationship(programmeId, programmeEpisodeId)
