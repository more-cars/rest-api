...
export async function getHasPrimeImageRelation(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelation = await ModelCar.getHasPrimeImageRelationship(modelCarId)
        ...
        if (e instanceof NodeNotFoundError) {
            ...
            const marshalledData = marshalEmptyRelation(ControllerNodeType.ModelCar, modelCarId, RelationType.ModelCarHasPrimeImage)
            return sendResponse200(marshalledData, res)
        } else {
            console.error(e)
