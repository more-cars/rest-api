...
export async function getAll(req: express.Request, res: express.Response) {
    try {
        const params = extractCollectionConstraintParameters(req, getNodeTypeSpecification(NodeType.ModelCar))
        const modelNodes = await ModelCar.findAll(params)
        const nodes = modelNodes.map(node => convertModelCarModelNodeToControllerNode(node))
        const totalAmount = await Node.getTotalAmount(mapControllerNodeTypeToModelNodeType(ControllerNodeType.ModelCar), params)
        const marshalledData = marshalNodeCollection(ControllerNodeType.ModelCar, nodes, params, totalAmount)
