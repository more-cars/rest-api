...
export async function getAllHasCarModelRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await Brand.getAllHasCarModelRelationships(brandId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.Brand, brandId, RelationType.BrandHasCarModel)
