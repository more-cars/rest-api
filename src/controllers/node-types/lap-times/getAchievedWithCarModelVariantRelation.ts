...
export async function getAchievedWithCarModelVariantRelation(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelation = await LapTime.getAchievedWithCarModelVariantRelationship(lapTimeId)
        ...
        if (e instanceof NodeNotFoundError) {
            ...
            const marshalledData = marshalEmptyRelation(ControllerNodeType.LapTime, lapTimeId, RelationType.LapTimeAchievedWithCarModelVariant)
            return sendResponse200(marshalledData, res)
        } else {
            console.error(e)
