...
export async function getAllHasVideoRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await GamingPlatform.getAllHasVideoRelationships(gamingPlatformId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.GamingPlatform, gamingPlatformId, RelationType.GamingPlatformHasVideo)
