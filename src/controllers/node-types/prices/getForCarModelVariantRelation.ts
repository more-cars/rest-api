...
export async function getForCarModelVariantRelation(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelation = await Price.getForCarModelVariantRelationship(priceId)
        ...
        if (e instanceof NodeNotFoundError) {
            ...
            const marshalledData = marshalEmptyRelation(ControllerNodeType.Price, priceId, RelationType.PriceForCarModelVariant)
            return sendResponse200(marshalledData, res)
        } else {
            console.error(e)
