...
export async function getAllHasVideoRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await ModelCarBrand.getAllHasVideoRelationships(modelCarBrandId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.ModelCarBrand, modelCarBrandId, RelationType.ModelCarBrandHasVideo)
