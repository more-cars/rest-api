...
export async function createCreatedModelCarRelation(req: express.Request, res: express.Response) {
    const modelCarBrandId = parseInt(req.params.modelCarBrandId)
    const modelCarId = parseInt(req.params.modelCarId)

    try {
        const modelRelation = await ModelCarBrand.createCreatedModelCarRelationship(modelCarBrandId, modelCarId)
