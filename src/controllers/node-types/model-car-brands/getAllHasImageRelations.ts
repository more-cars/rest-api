...
export async function getAllHasImageRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await ModelCarBrand.getAllHasImageRelationships(modelCarBrandId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.ModelCarBrand, modelCarBrandId, RelationType.ModelCarBrandHasImage)
