...
export async function getAll(req: express.Request, res: express.Response) {
    try {
        const params = extractCollectionConstraintParameters(req, getNodeTypeSpecification(NodeType.ModelCarBrand))
        const modelNodes = await ModelCarBrand.findAll(params)
        const nodes = modelNodes.map(node => convertModelCarBrandModelNodeToControllerNode(node))
        const totalAmount = await Node.getTotalAmount(mapControllerNodeTypeToModelNodeType(ControllerNodeType.ModelCarBrand), params)
        const marshalledData = marshalNodeCollection(ControllerNodeType.ModelCarBrand, nodes, params, totalAmount)
