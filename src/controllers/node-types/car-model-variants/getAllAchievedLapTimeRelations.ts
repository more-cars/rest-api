...
export async function getAllAchievedLapTimeRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await CarModelVariant.getAllAchievedLapTimeRelationships(carModelVariantId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.CarModelVariant, carModelVariantId, RelationType.CarModelVariantAchievedLapTime)
