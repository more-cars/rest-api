...
export async function createHasScaleModelRelation(req: express.Request, res: express.Response) {
    const carModelVariantId = parseInt(req.params.carModelVariantId)
    const modelCarId = parseInt(req.params.modelCarId)

    try {
        const modelRelation = await CarModelVariant.createHasScaleModelRelationship(carModelVariantId, modelCarId)
