...
export async function createIsCoveredByBookRelation(req: express.Request, res: express.Response) {
    const carModelVariantId = parseInt(req.params.carModelVariantId)
    const bookId = parseInt(req.params.bookId)

    try {
        const modelRelation = await CarModelVariant.createIsCoveredByBookRelationship(carModelVariantId, bookId)
