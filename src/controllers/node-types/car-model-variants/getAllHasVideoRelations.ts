...
export async function getAllHasVideoRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await CarModelVariant.getAllHasVideoRelationships(carModelVariantId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.CarModelVariant, carModelVariantId, RelationType.CarModelVariantHasVideo)
