...
export async function getHasMainVideoRelation(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelation = await CarModelVariant.getHasMainVideoRelationship(carModelVariantId)
        ...
        if (e instanceof NodeNotFoundError) {
            ...
            const marshalledData = marshalEmptyRelation(ControllerNodeType.CarModelVariant, carModelVariantId, RelationType.CarModelVariantHasMainVideo)
            return sendResponse200(marshalledData, res)
        } else {
