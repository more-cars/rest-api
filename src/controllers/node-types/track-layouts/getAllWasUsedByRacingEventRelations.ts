...
export async function getAllWasUsedByRacingEventRelations(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelations = await TrackLayout.getAllWasUsedByRacingEventRelationships(trackLayoutId)
        const relations = modelRelations.map(relation => convertModelRelationToControllerRelation(relation))
        const marshalledData = marshalRelations(relations, ControllerNodeType.TrackLayout, trackLayoutId, RelationType.TrackLayoutWasUsedByRacingEvent)
