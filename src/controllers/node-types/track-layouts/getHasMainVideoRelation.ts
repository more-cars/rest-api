...
export async function getHasMainVideoRelation(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelation = await TrackLayout.getHasMainVideoRelationship(trackLayoutId)
        ...
        if (e instanceof NodeNotFoundError) {
            ...
            const marshalledData = marshalEmptyRelation(ControllerNodeType.TrackLayout, trackLayoutId, RelationType.TrackLayoutHasMainVideo)
            return sendResponse200(marshalledData, res)
        } else {
            console.error(e)
