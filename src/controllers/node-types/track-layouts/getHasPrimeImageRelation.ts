...
export async function getHasPrimeImageRelation(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelation = await TrackLayout.getHasPrimeImageRelationship(trackLayoutId)
        ...
        if (e instanceof NodeNotFoundError) {
            ...
            const marshalledData = marshalEmptyRelation(ControllerNodeType.TrackLayout, trackLayoutId, RelationType.TrackLayoutHasPrimeImage)
            return sendResponse200(marshalledData, res)
        } else {
            console.error(e)
