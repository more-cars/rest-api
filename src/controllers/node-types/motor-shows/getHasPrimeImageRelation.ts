...
export async function getHasPrimeImageRelation(req: express.Request, res: express.Response) {
    ...
    try {
        const modelRelation = await MotorShow.getHasPrimeImageRelationship(motorShowId)
        ...
        if (e instanceof NodeNotFoundError) {
            ...
            const marshalledData = marshalEmptyRelation(ControllerNodeType.MotorShow, motorShowId, RelationType.MotorShowHasPrimeImage)
            return sendResponse200(marshalledData, res)
        } else {
            console.error(e)
