...
import type {MotorShowInput} from "../../../models/node-types/motor-shows/types/MotorShowInput"
...
export async function updateNode(req: express.Request, res: express.Response) {
    ...
    const data = unmarshalInputData(req.body, propertyNames) as MotorShowInput

    if (!validateInputData(data, NodeType.MotorShow, 'UPDATE')) {
        return sendResponse400(res)
    }

    try {
        const modelNode = await MotorShow.update(nodeId, data)
