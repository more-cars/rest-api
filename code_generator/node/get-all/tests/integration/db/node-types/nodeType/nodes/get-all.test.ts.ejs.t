---
to: tests/integration/db/node-types/<%= h.changeCase.kebab(h.inflection.pluralize(nodeType)) %>/nodes/get-all.test.ts
---
...
test('When there are no <%= h.changeCase.upper(h.inflection.pluralize(nodeType)) %> then an empty array should be returned', async () => {
    ...
    const actual<%= h.changeCase.pascal(h.inflection.pluralize(nodeType)) %> = await fetchNodesFromDb(DbNodeType.<%= h.changeCase.pascal(nodeType) %>)
