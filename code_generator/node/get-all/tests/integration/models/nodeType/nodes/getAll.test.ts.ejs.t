---
to: tests/integration/models/<%= h.changeCase.kebab(h.inflection.pluralize(nodeType)) %>/nodes/getAll.test.ts
---
...
test('When there are no <%= h.changeCase.upper(h.inflection.pluralize(nodeType)) %> then an empty array should be returned', async () => {
    ...
    const actual<%= h.changeCase.pascal(h.inflection.pluralize(nodeType)) %> = await <%= h.changeCase.pascal(nodeType) %>.findAll()
