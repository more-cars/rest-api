---
to: tests/smoke/bruno/node-types/<%= h.changeCase.kebab(h.inflection.pluralize(startNodeType)) %>/Relation - Get specific ›<%= h.changeCase.kebab(relationshipName) %>‹ (invalid data).yml
---
info:
  name: Get specific ›<%= h.changeCase.kebab(relationshipName) %>‹ relation (invalid data)
  type: http
  tags:
    - get
    - relation
