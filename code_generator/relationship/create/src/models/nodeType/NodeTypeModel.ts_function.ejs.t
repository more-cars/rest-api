...
<% } else if (cardinality === '1:n') { %>
        await deleteDeprecatedRelationship(<%= h.changeCase.camel(endNodeType) %>Id, DbRelationship.<%= h.changeCase.pascal(startNodeType) %><%= h.changeCase.pascal(relationshipName) %>)
<% } %>
