...
npx ts-node "$SCRIPT_PATH"/lib/collect-db-restore-parameters.ts $TEMPORARY_ENV_FILE
. "$SCRIPT_PATH"/$TEMPORARY_ENV_FILE
rm "$SCRIPT_PATH"/env.sh

echo ----------------------------------------------------------
echo Restoring database with following configuration:
...
if [ "$TARGET_CLUSTER" = minikube ]; then
  ...
  kubectl apply -k "$SCRIPT_PATH"/../deployment/overlays/"$TARGET_ENVIRONMENT"/jobs/restore-db
