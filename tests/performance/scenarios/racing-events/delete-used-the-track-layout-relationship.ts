...
export const options = {
    ...
    scenarios: {
        deleteUsedTheTrackLayoutRelationship: {
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        const trackLayout = createNode(ControllerNodeType.TRACK_LAYOUT)
        createRelationship(
            ControllerNodeType.RACING_EVENT,
            racingEventId,
            trackLayout,
            'used the track layout',
        )
...
export default function (data: { racingEventId: number, trackLayoutIds: number[] }) {
    const trackLayoutId = data.trackLayoutIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/racing-events/${data.racingEventId}/used-the-track-layout/${trackLayoutId}`
