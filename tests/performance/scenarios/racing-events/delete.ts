...
import {createNode} from "../../_testdata/racing-events/createNode.ts"
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        racingEvents.push(createNode())
    }

    return {ids: racingEvents}
}

export default function (data: { ids: any[] }) {
    const id = data.ids[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/racing-events/${id}`
