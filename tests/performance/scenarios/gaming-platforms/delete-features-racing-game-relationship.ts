...
export const options = {
    ...
    scenarios: {
        deleteFeaturesRacingGameRelationship: {
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        const racingGame = createNode(NodeTypeEnum.RACING_GAME)
        createRelationship(
            NodeTypeEnum.GAMING_PLATFORM,
            gamingPlatformId,
            racingGame,
...
export default function (data: { gamingPlatformId: number, racingGameIds: number[] }) {
    const racingGameId = data.racingGameIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/gaming-platforms/${data.gamingPlatformId}/features-racing-game/${racingGameId}`
