...
export function setup() {
    const priceId = createNode(ControllerNodeType.Price)
...
export default function (data: { priceId: number, carModelVariantIds: number[] }) {
    const carModelVariantId = data.carModelVariantIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/prices/${data.priceId}/for-car-model-variant/${carModelVariantId}`
