...
import {createNode} from "../../_testdata/racing-series/createNode.ts"
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        racingSeries.push(createNode())
    }

    return {ids: racingSeries}
}

export default function (data: { ids: any[] }) {
    const id = data.ids[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/racing-series/${id}`
