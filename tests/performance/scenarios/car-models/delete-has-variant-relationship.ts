...
export default function (data: { carModelId: number, carModelVariantIds: number[] }) {
    const carModelVariantId = data.carModelVariantIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/car-models/${data.carModelId}/has-variant/${carModelVariantId}`
