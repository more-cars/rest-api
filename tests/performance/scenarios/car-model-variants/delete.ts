...
import {createNode} from "../../_testdata/car-model-variants/createNode.ts"
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        carModelVariants.push(createNode())
    }

    return {ids: carModelVariants}
}

export default function (data: { ids: any[] }) {
    const id = data.ids[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/car-model-variants/${id}`
