...
export const options = {
    ...
    scenarios: {
        deleteIsCoveredByBookRelationship: {
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        const book = createNode(ControllerNodeType.Book)
        createRelationship(
...
export default function (data: { carModelVariantId: number, bookIds: number[] }) {
    const bookId = data.bookIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/car-model-variants/${data.carModelVariantId}/is-covered-by-book/${bookId}`
