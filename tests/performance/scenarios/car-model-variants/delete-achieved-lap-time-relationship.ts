...
export const options = {
    ...
    scenarios: {
        deleteAchievedLapTimeRelationship: {
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        const lapTime = createNode(NodeTypeEnum.LAP_TIME)
        createRelationship(
...
export default function (data: { carModelVariantId: number, lapTimeIds: number[] }) {
    const lapTimeId = data.lapTimeIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/car-model-variants/${data.carModelVariantId}/achieved-lap-time/${lapTimeId}`
