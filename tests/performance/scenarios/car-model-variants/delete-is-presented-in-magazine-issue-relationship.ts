...
export const options = {
    ...
    scenarios: {
        deleteIsPresentedInMagazineIssueRelationship: {
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        ...
        createRelationship(
            ...
            carModelVariantId,
            magazineIssue,
            'is presented in magazine issue',
        )
...
export default function (data: { carModelVariantId: number, magazineIssueIds: number[] }) {
    const magazineIssueId = data.magazineIssueIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/car-model-variants/${data.carModelVariantId}/is-presented-in-magazine-issue/${magazineIssueId}`
