...
export const options = {
    ...
    scenarios: {
        deleteReviewedByMagazineIssueWithRatingRelationship: {
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        ...
        createRelationship(
            ...
            carModelVariantId,
            rating,
            'reviewed by magazine issue with rating',
        )
...
export default function (data: { carModelVariantId: number, ratingIds: number[] }) {
    const ratingId = data.ratingIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/car-model-variants/${data.carModelVariantId}/reviewed-by-magazine-issue-with-rating/${ratingId}`
