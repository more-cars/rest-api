...
export const options = {
    ...
    scenarios: {
        deleteFeaturedInProgrammeEpisodeRelationship: {
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        ...
        createRelationship(
            ...
            carModelVariantId,
            programmeEpisode,
            'featured in programme episode',
        )
        programmeEpisodeIds.push(programmeEpisode)
    }
...
export default function (data: { carModelVariantId: number, programmeEpisodeIds: number[] }) {
    const programmeEpisodeId = data.programmeEpisodeIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/car-model-variants/${data.carModelVariantId}/featured-in-programme-episode/${programmeEpisodeId}`
