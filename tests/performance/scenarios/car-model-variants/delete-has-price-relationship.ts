...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        const price = createNode(ControllerNodeType.Price)
        createRelationship(
...
export default function (data: { carModelVariantId: number, priceIds: number[] }) {
    const priceId = data.priceIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/car-model-variants/${data.carModelVariantId}/has-price/${priceId}`
