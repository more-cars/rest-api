...
export const options = {
    ...
    scenarios: {
        deleteIsMainVideoOfNodeRelationship: {
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        const node = createCarModel()
        createRelationship(
            ControllerNodeType.Video,
            videoId,
            node,
            'is main video of node',
        )
        nodeIds.push(node)
    }
...
export default function (data: { videoId: number, nodeIds: number[] }) {
    const nodeId = data.nodeIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/videos/${data.videoId}/is-main-video-of-node/${nodeId}`
