...
import {createNode} from "../../_testdata/racing-sessions/createNode.ts"
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        racingSessions.push(createNode())
    }

    return {ids: racingSessions}
}

export default function (data: { ids: any[] }) {
    const id = data.ids[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/racing-sessions/${id}`
