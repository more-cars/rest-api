...
export default function (data: { racingSessionId: number, imageIds: number[] }) {
    const imageId = data.imageIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/racing-sessions/${data.racingSessionId}/has-prime-image/${imageId}`
