...
export default function (data: { magazineIssueId: number, imageIds: number[] }) {
    const imageId = data.imageIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/magazine-issues/${data.magazineIssueId}/has-prime-image/${imageId}`
