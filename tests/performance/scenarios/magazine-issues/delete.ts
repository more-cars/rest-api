...
import {createNode} from "../../_testdata/magazine-issues/createNode.ts"
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        magazineIssues.push(createNode())
    }

    return {ids: magazineIssues}
}

export default function (data: { ids: any[] }) {
    const id = data.ids[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/magazine-issues/${id}`
