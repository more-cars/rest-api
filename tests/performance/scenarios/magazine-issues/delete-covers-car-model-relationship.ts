...
export default function (data: { magazineIssueId: number, carModelIds: number[] }) {
    const carModelId = data.carModelIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/magazine-issues/${data.magazineIssueId}/covers-car-model/${carModelId}`
