...
export default function (data: { magazineIssueId: number, carModelVariantIds: number[] }) {
    const carModelVariantId = data.carModelVariantIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/magazine-issues/${data.magazineIssueId}/presents-car-model-variant/${carModelVariantId}`
