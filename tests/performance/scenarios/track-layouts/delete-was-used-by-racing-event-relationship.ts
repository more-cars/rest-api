...
export const options = {
    ...
    scenarios: {
        deleteWasUsedByRacingEventRelationship: {
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        const racingEvent = createNode(ControllerNodeType.RACING_EVENT)
        createRelationship(
            ControllerNodeType.TRACK_LAYOUT,
            trackLayoutId,
            racingEvent,
            'was used by racing event',
        )
...
export default function (data: { trackLayoutId: number, racingEventIds: number[] }) {
    const racingEventId = data.racingEventIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/track-layouts/${data.trackLayoutId}/was-used-by-racing-event/${racingEventId}`
