...
import {createNode} from "../../_testdata/track-layouts/createNode.ts"
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        trackLayouts.push(createNode())
    }

    return {ids: trackLayouts}
}

export default function (data: { ids: any[] }) {
    const id = data.ids[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/track-layouts/${id}`
