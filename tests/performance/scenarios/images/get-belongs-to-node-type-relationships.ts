...
export function setup() {
    ...
    createImageBelongsToNodeRelationships(imageId, 3)
...
export default function (data: { imageId: number }) {
    const url = `${__ENV.API_URL}/images/${data.imageId}/belongs-to-node-type`
    ...
    check(response, {
        ...
        'response is an Array': (r) => r.json().car_models.length > 0,
    })
