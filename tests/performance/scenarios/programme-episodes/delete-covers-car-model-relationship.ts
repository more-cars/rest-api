...
export const options = {
    ...
    scenarios: {
        deleteCoversCarModelRelationship: {
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        const carModel = createNode(ControllerNodeType.CarModel)
        createRelationship(
...
export default function (data: { programmeEpisodeId: number, carModelIds: number[] }) {
    const carModelId = data.carModelIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/programme-episodes/${data.programmeEpisodeId}/covers-car-model/${carModelId}`
