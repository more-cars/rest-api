...
export default function (data: { programmeEpisodeId: number, imageIds: number[] }) {
    const imageId = data.imageIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/programme-episodes/${data.programmeEpisodeId}/has-prime-image/${imageId}`
