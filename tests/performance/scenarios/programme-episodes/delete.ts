...
import {createNode} from "../../_testdata/programme-episodes/createNode.ts"
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        programmeEpisodes.push(createNode())
    }

    return {ids: programmeEpisodes}
}

export default function (data: { ids: any[] }) {
    const id = data.ids[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/programme-episodes/${id}`
