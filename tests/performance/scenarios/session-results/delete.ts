...
import {createNode} from "../../_testdata/session-results/createNode.ts"
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        sessionResults.push(createNode())
    }

    return {ids: sessionResults}
}

export default function (data: { ids: any[] }) {
    const id = data.ids[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/session-results/${id}`
