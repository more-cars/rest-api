...
export default function (data: { sessionResultId: number, imageIds: number[] }) {
    const imageId = data.imageIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/session-results/${data.sessionResultId}/has-prime-image/${imageId}`
