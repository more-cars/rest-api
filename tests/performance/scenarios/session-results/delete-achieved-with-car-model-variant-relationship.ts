...
export default function (data: { sessionResultId: number, carModelVariantIds: number[] }) {
    const carModelVariantId = data.carModelVariantIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/session-results/${data.sessionResultId}/achieved-with-car-model-variant/${carModelVariantId}`
