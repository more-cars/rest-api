...
export const options = {
    ...
    scenarios: {
        deleteIsScaleModelOfCarModelVariantRelationship: {
...
export function setup() {
    ...
    for (let i = 0; i < 310; i++) {
        ...
        createRelationship(
            ...
            modelCarId,
            carModelVariant,
            'is scale model of car model variant',
        )
...
export default function (data: { modelCarId: number, carModelVariantIds: number[] }) {
    const carModelVariantId = data.carModelVariantIds[exec.scenario.iterationInTest]
    const url = `${__ENV.API_URL}/model-cars/${data.modelCarId}/is-scale-model-of-car-model-variant/${carModelVariantId}`
