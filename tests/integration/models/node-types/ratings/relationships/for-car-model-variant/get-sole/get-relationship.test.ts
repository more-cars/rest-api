...
describe('Requesting a ›for-car-model-variant‹ relationship', () => {
    test('node and relationship exist', async () => {
        const expectedRelationship = await seedRelationship(DbNodeType.Rating, DbNodeType.CarModelVariant, RelationshipType.RatingForCarModelVariant)
        ...
        const actualRelationship = await Rating.getForCarModelVariantRelationship(expectedRatingId)
    ...
    test('node exists, but not the relationship', async () => {
        ...
        await expect(Rating.getForCarModelVariantRelationship(rating.properties.id))
            .rejects
            .toThrow(RelNotFoundError)
    })

    test('neither node, nor relationship exist', async () => {
        await expect(Rating.getForCarModelVariantRelationship(-42))
            .rejects
