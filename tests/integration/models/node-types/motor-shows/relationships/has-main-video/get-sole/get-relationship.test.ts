...
describe('Requesting a ›has-main-video‹ relationship', () => {
    test('node and relationship exist', async () => {
        const expectedRelationship = await seedRelationship(DbNodeType.MotorShow, DbNodeType.Video, RelationshipType.MotorShowHasMainVideo)
        ...
        const actualRelationship = await MotorShow.getHasMainVideoRelationship(expectedMotorShowId)
    ...
    test('node exists, but not the relationship', async () => {
        ...
        await expect(MotorShow.getHasMainVideoRelationship(motorShow.properties.id))
            .rejects
            .toThrow(RelNotFoundError)
    })

    test('neither node, nor relationship exist', async () => {
        await expect(MotorShow.getHasMainVideoRelationship(-42))
            .rejects
