...
describe('Deleting a ›has-main-video‹ relationship', () => {
    test('TRACK LAYOUT node does not exist', async () => {
        ...
        await expect(TrackLayout.deleteHasMainVideoRelationship(trackLayout.properties.id, -43))
            .rejects
    ...
    test('VIDEO node does not exist', async () => {
        ...
        await expect(TrackLayout.deleteHasMainVideoRelationship(-42, video.properties.id))
            .rejects
            .toThrow(NodeNotFoundError)
    })

    test('TRACK LAYOUT node and VIDEO node do not exist', async () => {
        await expect(TrackLayout.deleteHasMainVideoRelationship(-42, -43))
            .rejects
            .toThrow(NodeNotFoundError)
    })
    ...
    test('both nodes exist, but have no ›has-main-video‹ relationship', async () => {
        ...
        await expect(TrackLayout.deleteHasMainVideoRelationship(trackLayout.properties.id, video.properties.id))
            .rejects
    ...
    test('both nodes exist and have a ›has-main-video‹ relationship', async () => {
        ...
        await TrackLayout.deleteHasMainVideoRelationship(seededRelationship.start_node.properties.id, seededRelationship.end_node.properties.id)
