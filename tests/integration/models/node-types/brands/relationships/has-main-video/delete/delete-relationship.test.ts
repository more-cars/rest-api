...
describe('Deleting a ›has-main-video‹ relationship', () => {
    test('BRAND node does not exist', async () => {
        ...
        await expect(Brand.deleteHasMainVideoRelationship(brand.properties.id, -43))
            .rejects
    ...
    test('VIDEO node does not exist', async () => {
        ...
        await expect(Brand.deleteHasMainVideoRelationship(-42, video.properties.id))
            .rejects
            .toThrow(NodeNotFoundError)
    })

    test('BRAND node and VIDEO node do not exist', async () => {
        await expect(Brand.deleteHasMainVideoRelationship(-42, -43))
            .rejects
            .toThrow(NodeNotFoundError)
    })
    ...
    test('both nodes exist, but have no ›has-main-video‹ relationship', async () => {
        ...
        await expect(Brand.deleteHasMainVideoRelationship(brand.properties.id, video.properties.id))
            .rejects
    ...
    test('both nodes exist and have a ›has-main-video‹ relationship', async () => {
        ...
        await Brand.deleteHasMainVideoRelationship(seededRelationship.start_node.properties.id, seededRelationship.end_node.properties.id)
