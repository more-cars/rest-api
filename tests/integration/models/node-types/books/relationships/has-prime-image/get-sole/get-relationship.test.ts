...
describe('Requesting a ›has-prime-image‹ relationship', () => {
    test('node and relationship exist', async () => {
        const expectedRelationship = await seedRelationship(DbNodeType.Book, DbNodeType.Image, RelationshipType.BookHasPrimeImage)
        ...
        const actualRelationship = await Book.getHasPrimeImageRelationship(expectedBookId)
    ...
    test('node exists, but not the relationship', async () => {
        ...
        await expect(Book.getHasPrimeImageRelationship(book.properties.id))
            .rejects
            .toThrow(RelNotFoundError)
    })

    test('neither node, nor relationship exist', async () => {
        await expect(Book.getHasPrimeImageRelationship(-42))
            .rejects
