...
describe('Deleting a ›has-prime-image‹ relationship', () => {
    test('MAGAZINE node does not exist', async () => {
        ...
        await expect(Magazine.deleteHasPrimeImageRelationship(magazine.properties.id, -43))
            .rejects
    ...
    test('IMAGE node does not exist', async () => {
        ...
        await expect(Magazine.deleteHasPrimeImageRelationship(-42, image.properties.id))
            .rejects
            .toThrow(NodeNotFoundError)
    })

    test('MAGAZINE node and IMAGE node do not exist', async () => {
        await expect(Magazine.deleteHasPrimeImageRelationship(-42, -43))
            .rejects
            .toThrow(NodeNotFoundError)
    })
    ...
    test('both nodes exist, but have no ›has-prime-image‹ relationship', async () => {
        ...
        await expect(Magazine.deleteHasPrimeImageRelationship(magazine.properties.id, image.properties.id))
            .rejects
    ...
    test('both nodes exist and have a ›has-prime-image‹ relationship', async () => {
        ...
        await Magazine.deleteHasPrimeImageRelationship(seededRelationship.start_node.properties.id, seededRelationship.end_node.properties.id)
