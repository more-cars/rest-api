...
describe('Requesting a ›has-prime-image‹ relationship', () => {
    test('node and relationship exist', async () => {
        const expectedRelationship = await seedRelationship(DbNodeType.Price, DbNodeType.Image, RelationshipType.PriceHasPrimeImage)
        ...
        const actualRelationship = await Price.getHasPrimeImageRelationship(expectedPriceId)
    ...
    test('node exists, but not the relationship', async () => {
        ...
        await expect(Price.getHasPrimeImageRelationship(price.properties.id))
            .rejects
            .toThrow(RelNotFoundError)
    })

    test('neither node, nor relationship exist', async () => {
        await expect(Price.getHasPrimeImageRelationship(-42))
            .rejects
