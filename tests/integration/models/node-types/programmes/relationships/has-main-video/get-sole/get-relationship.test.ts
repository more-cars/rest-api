...
describe('Requesting a ›has-main-video‹ relationship', () => {
    test('node and relationship exist', async () => {
        const expectedRelationship = await seedRelationship(DbNodeType.Programme, DbNodeType.Video, RelationshipType.ProgrammeHasMainVideo)
        ...
        const actualRelationship = await Programme.getHasMainVideoRelationship(expectedProgrammeId)
    ...
    test('node exists, but not the relationship', async () => {
        ...
        await expect(Programme.getHasMainVideoRelationship(programme.properties.id))
            .rejects
            .toThrow(RelNotFoundError)
    })

    test('neither node, nor relationship exist', async () => {
        await expect(Programme.getHasMainVideoRelationship(-42))
            .rejects
