...
describe('Deleting a ›has-main-video‹ relationship', () => {
    test('RACING EVENT node does not exist', async () => {
        ...
        await expect(RacingEvent.deleteHasMainVideoRelationship(racingEvent.properties.id, -43))
            .rejects
    ...
    test('VIDEO node does not exist', async () => {
        ...
        await expect(RacingEvent.deleteHasMainVideoRelationship(-42, video.properties.id))
            .rejects
            .toThrow(NodeNotFoundError)
    })

    test('RACING EVENT node and VIDEO node do not exist', async () => {
        await expect(RacingEvent.deleteHasMainVideoRelationship(-42, -43))
            .rejects
            .toThrow(NodeNotFoundError)
    })
    ...
    test('both nodes exist, but have no ›has-main-video‹ relationship', async () => {
        ...
        await expect(RacingEvent.deleteHasMainVideoRelationship(racingEvent.properties.id, video.properties.id))
            .rejects
    ...
    test('both nodes exist and have a ›has-main-video‹ relationship', async () => {
        ...
        await RacingEvent.deleteHasMainVideoRelationship(seededRelationship.start_node.properties.id, seededRelationship.end_node.properties.id)
