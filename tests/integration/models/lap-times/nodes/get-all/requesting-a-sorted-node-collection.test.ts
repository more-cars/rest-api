...
describe('A sorted "get all LAP TIME nodes" request returns the nodes in correct order', () => {
    ...
    test('when there exist LAP TIME nodes', async () => {
        ...
        expect(ascNodes[0].attributes.driver_name === nodeA.attributes.driver_name)
        expect(ascNodes[1].attributes.driver_name === nodeB.attributes.driver_name)
        expect(ascNodes[2].attributes.driver_name === nodeC.attributes.driver_name)

        const descNodes = await LapTime.findAll({sortByProperty: 'driver_name', sortDirection: 'desc'})
        expect(descNodes.length).toEqual(3)
        expect(descNodes[0].attributes.driver_name === nodeC.attributes.driver_name)
        expect(descNodes[1].attributes.driver_name === nodeB.attributes.driver_name)
        expect(descNodes[2].attributes.driver_name === nodeA.attributes.driver_name)
    })
})
