...
describe('A sorted "get all SESSION RESULT nodes" request returns the nodes in correct order', () => {
    ...
    test('when there exist SESSION RESULT nodes', async () => {
        ...
        expect(ascNodes[0].attributes.position === nodeA.attributes.position)
        expect(ascNodes[1].attributes.position === nodeB.attributes.position)
        expect(ascNodes[2].attributes.position === nodeC.attributes.position)

        const descNodes = await SessionResult.findAll({sortByProperty: 'position', sortDirection: 'desc'})
        expect(descNodes.length).toEqual(3)
        expect(descNodes[0].attributes.position === nodeC.attributes.position)
        expect(descNodes[1].attributes.position === nodeB.attributes.position)
        expect(descNodes[2].attributes.position === nodeA.attributes.position)
    })
})
