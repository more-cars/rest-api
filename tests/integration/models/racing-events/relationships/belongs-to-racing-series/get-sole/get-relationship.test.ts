...
describe('Requesting a ›belongs-to-racing-series‹ relationship', () => {
    test('node and relationship exist', async () => {
        const expectedRelationship = await seedRelationship(NodeTypeEnum.RACING_EVENT, NodeTypeEnum.RACING_SERIES, DbRelationship.RacingEventBelongsToRacingSeries, REL.REVERSE)
    ...
    test('node exists, but not the relationship', async () => {
        ...
        await expect(RacingEvent.getBelongsToRacingSeriesRelationship(racingEvent.id))
            .rejects
