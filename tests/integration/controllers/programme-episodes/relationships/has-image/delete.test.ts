...
describe('Deleting a ›has-image‹ relationship', () => {
    test('Providing valid data', async () => {
        ProgrammeEpisode.deleteHasImageRelationship = vi.fn().mockReturnValue(null)

        const response = await request(app)
            .delete('/programme-episodes/123/has-image/567')
    ...
    test('Providing invalid data (non-existent nodes)', async () => {
        vi.spyOn(ProgrammeEpisode, 'deleteHasImageRelationship')
