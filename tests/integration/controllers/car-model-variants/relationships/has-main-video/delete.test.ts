...
describe('Deleting a ›has-main-video‹ relationship', () => {
    test('Providing valid data', async () => {
        CarModelVariant.deleteHasMainVideoRelationship = vi.fn().mockReturnValue(null)

        const response = await request(app)
            .delete('/car-model-variants/123/has-main-video/567')
    ...
    test('Providing invalid data (non-existent nodes)', async () => {
        vi.spyOn(CarModelVariant, 'deleteHasMainVideoRelationship')
