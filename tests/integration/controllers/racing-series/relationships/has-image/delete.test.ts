...
describe('Deleting a ›has-image‹ relationship', () => {
    test('Providing valid data', async () => {
        RacingSeries.deleteHasImageRelationship = vi.fn().mockReturnValue(null)

        const response = await request(app)
            .delete('/racing-series/123/has-image/567')
    ...
    test('Providing invalid data (non-existent nodes)', async () => {
        vi.spyOn(RacingSeries, 'deleteHasImageRelationship')
