...
describe('Deleting a ›has-video‹ relationship', () => {
    test('Providing valid data', async () => {
        RacingSeries.deleteHasVideoRelationship = vi.fn().mockReturnValue(null)

        const response = await request(app)
            .delete('/racing-series/123/has-video/567')
    ...
    test('Providing invalid data (non-existent nodes)', async () => {
        vi.spyOn(RacingSeries, 'deleteHasVideoRelationship')
