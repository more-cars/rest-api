...
describe('Deleting a ›has-prime-image‹ relationship', () => {
    test('Providing valid data', async () => {
        RacingSeries.deleteHasPrimeImageRelationship = vi.fn().mockReturnValue(null)

        const response = await request(app)
            .delete('/racing-series/123/has-prime-image/567')
    ...
    test('Providing invalid data (non-existent nodes)', async () => {
        vi.spyOn(RacingSeries, 'deleteHasPrimeImageRelationship')
