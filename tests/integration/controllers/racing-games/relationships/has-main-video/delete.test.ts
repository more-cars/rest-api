...
describe('Deleting a ›has-main-video‹ relationship', () => {
    test('Providing valid data', async () => {
        RacingGame.deleteHasMainVideoRelationship = vi.fn().mockReturnValue(null)

        const response = await request(app)
            .delete('/racing-games/123/has-main-video/567')
    ...
    test('Providing invalid data (non-existent nodes)', async () => {
        vi.spyOn(RacingGame, 'deleteHasMainVideoRelationship')
