...
describe('Deleting a ›has-issue‹ relationship', () => {
    test('Providing valid data', async () => {
        Magazine.deleteHasIssueRelationship = vi.fn().mockReturnValue(null)

        const response = await request(app)
            .delete('/magazines/123/has-issue/567')
    ...
    test('Providing invalid data (non-existent nodes)', async () => {
        vi.spyOn(Magazine, 'deleteHasIssueRelationship')
    ...
    test('Providing invalid data (non-existent relationship)', async () => {
        vi.spyOn(Magazine, 'deleteHasIssueRelationship')
            .mockImplementation(async () => {
                throw new RelNotFoundError('has-issue', 123, 567)
            })
