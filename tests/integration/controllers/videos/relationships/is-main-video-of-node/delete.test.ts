import {describe, expect, test, vi} from 'vitest'
import request from 'supertest'
import {app} from '../../../../../../src/app'
import {Video} from "../../../../../../src/models/node-types/videos/Video"
import {NodeNotFoundError} from "../../../../../../src/models/types/NodeNotFoundError"
import {RelNotFoundError} from "../../../../../../src/models/types/RelNotFoundError"

describe('Deleting a ›is-main-video-of-node‹ relationship', () => {
    test('Providing valid data', async () => {
        Video.deleteIsMainVideoOfNodeRelationship = vi.fn().mockReturnValue(null)

        const response = await request(app)
            .delete('/videos/123/is-main-video-of-node/567')

        expect(response.statusCode)
            .toBe(204)
    })

    test('Providing invalid data (non-existent nodes)', async () => {
        vi.spyOn(Video, 'deleteIsMainVideoOfNodeRelationship')
            .mockImplementation(async () => {
                throw new NodeNotFoundError(123)
            })

        const response = await request(app)
            .delete('/videos/123/is-main-video-of-node/567')

        expect(response.statusCode)
            .toBe(404)
    })

    test('Providing invalid data (non-existent relationship)', async () => {
        vi.spyOn(Video, 'deleteIsMainVideoOfNodeRelationship')
            .mockImplementation(async () => {
                throw new RelNotFoundError('is-main-video-of-node', 123, 567)
            })

        const response = await request(app)
            .delete('/videos/123/is-main-video-of-node/567')

        expect(response.statusCode)
            .toBe(404)
    })

    test('Providing valid data, but the database call randomly fails', async () => {
        vi.spyOn(Video, 'deleteIsMainVideoOfNodeRelationship')
            .mockImplementation(async () => {
                throw new Error('Arbitrary error')
            })

        const response = await request(app)
            .delete('/videos/123/is-main-video-of-node/567')

        expect(response.statusCode)
            .toBe(500)
    })
})
