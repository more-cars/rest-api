...
test('Deleting the node when it actually exists', async () => {
    Video.delete = vi.fn().mockReturnValue(true)
...
test('Input is valid, but something breaks on the way', async () => {
    Video.delete = vi.fn().mockImplementation(() => {
        throw new Error()
    })

    const response = await request(app)
        .delete('/videos/987654321') // the actual ID is irrelevant here
