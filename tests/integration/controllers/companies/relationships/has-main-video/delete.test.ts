...
describe('Deleting a ›has-main-video‹ relationship', () => {
    test('Providing valid data', async () => {
        Company.deleteHasMainVideoRelationship = vi.fn().mockReturnValue(null)

        const response = await request(app)
            .delete('/companies/123/has-main-video/567')
    ...
    test('Providing invalid data (non-existent nodes)', async () => {
        vi.spyOn(Company, 'deleteHasMainVideoRelationship')
