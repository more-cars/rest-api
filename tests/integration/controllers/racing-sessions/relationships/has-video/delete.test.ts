...
describe('Deleting a ›has-video‹ relationship', () => {
    test('Providing valid data', async () => {
        RacingSession.deleteHasVideoRelationship = vi.fn().mockReturnValue(null)

        const response = await request(app)
            .delete('/racing-sessions/123/has-video/567')
    ...
    test('Providing invalid data (non-existent nodes)', async () => {
        vi.spyOn(RacingSession, 'deleteHasVideoRelationship')
