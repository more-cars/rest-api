...
describe('Deleting a ›has-video‹ relationship', () => {
    test('Providing valid data', async () => {
        MagazineIssue.deleteHasVideoRelationship = vi.fn().mockReturnValue(null)

        const response = await request(app)
            .delete('/magazine-issues/123/has-video/567')
    ...
    test('Providing invalid data (non-existent nodes)', async () => {
        vi.spyOn(MagazineIssue, 'deleteHasVideoRelationship')
