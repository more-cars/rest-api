...
describe('Deleting a ›for-car-model-variant‹ relationship', () => {
    test('Providing valid data', async () => {
        Price.deleteForCarModelVariantRelationship = vi.fn().mockReturnValue(null)

        const response = await request(app)
            .delete('/prices/123/for-car-model-variant/567')
    ...
    test('Providing invalid data (non-existent nodes)', async () => {
        vi.spyOn(Price, 'deleteForCarModelVariantRelationship')
