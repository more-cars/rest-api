...
describe('Deleting a ›has-layout‹ relationship', () => {
    test('Providing valid data', async () => {
        RaceTrack.deleteHasLayoutRelationship = vi.fn().mockReturnValue(null)

        const response = await request(app)
            .delete('/race-tracks/123/has-layout/567')
    ...
    test('Providing invalid data (non-existent nodes)', async () => {
        vi.spyOn(RaceTrack, 'deleteHasLayoutRelationship')
    ...
    test('Providing invalid data (non-existent relationship)', async () => {
        vi.spyOn(RaceTrack, 'deleteHasLayoutRelationship')
            .mockImplementation(async () => {
                throw new RelationshipNotFoundError('has-layout', 123, 567)
            })
