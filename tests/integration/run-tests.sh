...
if [ "$TEST_RUNNER" = local ]; then
  if [ "$CODE_COVERAGE_ENABLED" = true ]; then
    npx vitest run -c tests/integration/vite.config.ts --coverage
  else
    npx vitest run -c tests/integration/vite.config.ts
  fi
  ...
  kubectl apply -k "$SCRIPT_PATH"/../../deployment/overlays/"$TARGET_ENVIRONMENT"/jobs/integration-test
